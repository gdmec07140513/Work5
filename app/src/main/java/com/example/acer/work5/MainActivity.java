package com.example.acer.work5;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        player.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
            ...
            public void onPrepared(MediaPlayer mp) {
                ...
                vHeith = player.getVideoHeight();

                if (vWidth > currDisplay.getWidth() || vHeith > currDisplay.getHeight()) {
                    float wRatio = (float) vWidth / (float) currDisplay.getWidth();
                    float hRatio = (float) vHeith / (float) currDisplay.getHeight();

                    float ratio = Math.max(wRatio, hRatio);
                    vWidth = (int) Math.ceil((float) vWidth / ratio);
                    vHeith = (int) Math.ceil((float) vHeith / ratio);
                    surfaceView.setLayoutParams(new LinearLayout.LayoutParams(vWidth, vHeith));
